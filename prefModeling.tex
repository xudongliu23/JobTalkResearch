...
\frameT{Compact Representation of P-trees}{
	...
	\begin{enumerate}
	  ...
	  \item every non-leaf node $t$ labeled with $\varphi$ has either
	        two outgoing edges (\figref{compact1}), or one 
	    		outgoing edge pointing straight-down (\figref{compact4}),
					left (\figref{compact3}), or right (\figref{compact2}).
	\end{enumerate}
	...
	\begin{figure}
      ...
      \begin{tikzpicture}[->,>=stealth',
	      ...
	      level 2/.style={sibling distance=0.7cm, level distance=27pt}
	  	  ...
	  	  \node[main node,inner sep=3pt] (1) {S};
	  	  \node[rectangle,draw,inner sep=2pt] at (1,0) {$h\!\! >\!\! m\!\! >\!\! l$};
	
	  	  \node[main node,inner sep=3pt] (2) [below of=1] {C};
...
\frameT{Partial Lexicographic Preference Trees (PLP-Tree)}{
	\begin{figure}
      ...
      \begin{tikzpicture}[->,>=stealth',
        ...
        \node [main node,inner sep=3pt] (1){S}
          child {node [main node,inner sep=2pt] (2) {C}
            child {node [main node,inner sep=2pt] (4) {L} edge from parent node[left]  {\tiny{5}}}
            ...
            child {node [main node,inner sep=2pt] (5) {L} edge from parent node[right] {\tiny{2}}}
          edge from parent node[left] {\small{h}}}
          child {node [rectangle,draw] (6) {} edge from parent node[left] {\small{m}}}
          child {node [main node,inner sep=2pt] (10) {L}
            child {node [main node,inner sep=2pt] (11) {C} edge from parent node[left]  {\tiny{b}}}
            child {node [main node,inner sep=2pt] (12) {C} 
            	child {node [rectangle,draw] (17) {} edge from parent node[left]  {\tiny{5}}}
            	child {node [rectangle,draw] (18) {} edge from parent node[left]  {\tiny{7}}}
            	child {node [rectangle,draw] (19) {} edge from parent node[right] {\tiny{2}}}
						edge from parent node[left]  {\tiny{m}}}
            child {node [main node,inner sep=2pt] (13) {C} edge from parent node[right] {\tiny{s}}}
          edge from parent node[right] {\small{l}}};
      \end{tikzpicture}
			\caption{Full}
		\end{subfigure}
	...
	\begin{figure}
      ...
      \begin{tikzpicture}[->,>=stealth',
	      ...
	      level 2/.style={sibling distance=0.7cm, level distance=27pt}
	  	  ...
	  	  \node[main node,inner sep=2pt] (3) [below of=2] {L};
	    	\node[rectangle split, rectangle split parts=3, draw,inner sep=2pt,font=\sffamily\small] at (-1.8,-2)
	    	    {
							$2\!:\!b \!\!>\!\! m\!\!>\!\! s$
	    	      \nodepart{second}
							$5\!:\!m \!\!>\!\! 5\!\!>\!\! 2$
  ...
  \begin{enumerate}
    ...
    \begin{itemize}
      \item each attribute appears \tbf{exactly once} on every path from the root to a leaf.
			\item Unlike P-trees and PLP-trees, an LP-tree induces a total order.
    \end{itemize}
  \end{enumerate}
}
