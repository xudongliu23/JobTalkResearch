...
{
	\begin{enumerate}
		\item An \tit{LP tree} $\cL$ over 
					$\cI=\{X_1,\ldots,X_p\}$ is a (\tit{binary}) tree, where
		\begin{itemize}
			\item each node $t$ in $\cL$ is labeled by an issue from $\cI$
						and with \tit{preference information}, and
			\item each issue appears \tbf{exactly once} on every path from the root to a leaf.
		\end{itemize}
	\end{enumerate}
}
...
\frameT{Complexity of the Evaluation Problem: $(k,l)$-Approval}
{
	...
	\fnt{
		Liu and Truszczynski,
		\tit{Aggregating Conditionally Lexicographic Preferences Using Answer Set Programming},
		ADT, 2013.
	}
...
\frameT{Modeling the Problems in ASP}
{
	...
	\begin{itemize}
		\item Solvers: \tit{clingo}\footfullcite{Gebser:clingo}, 
					\tit{clingcon}\footfullcite{Ostrowski:clingcon}
	\end{itemize}
}
...
\frameT{Modeling the Problems in W-MAXSAT}
{
	...
	\begin{itemize}
		\item Solver: \tit{toulbar}\footfullcite{sanchez2008max}
	\end{itemize}
}
...
\frameT{Personalization in Trip Planning}{
	\begin{enumerate}
		...
		\item Developed a hipergraph-based trip planner that accommodates constraints specified as
					...
					to compute optimal routes using A*\footfullcite{abs/parc/Liu}.
